...
class SchoolPortal:
    ...
    def __init__(self, root):
        ...
        self.photo = PhotoImage(file = 'assests/logo.png')
        ...
        Label(frame, text='Email-ID:').grid(row=4, column=1, sticky=W)
    ...
    #-------------------------Add new Record---------------------------
    def validation(self):
        return len(self.firstname.get())!=0 and len(self.lastname.get())!=0 and len(self.username.get())!=0 and len(self.lastname.get())!=0 and len(self.email.get())!=0 and len(self.subject.get())!=0 and len(self.age.get())!=0
    ...
    def add_records(self):
        if self.validation():
            ...
            self.message['text'] = 'Incomplete Information'
    ...
    def dele(self):
        de = tkinter.messagebox.askquestion('Delete Record', ' Are you sure to delete record?')
        if de == 'yes':
            self.delete_record()
    ...
    def edit_box(self):
        ...
        Label(self.edit_root, text='Old Email-ID').grid(row=6, column=1, sticky=W)
        Entry(self.edit_root, textvariable=StringVar(self.edit_root, value=email), state='readonly').grid(row=6,column=2)
        Label(self.edit_root, text='New Email-ID').grid(row=7, column=1, sticky=W)
        ...
        Button(self.edit_root, text='Save Updated Record', command = lambda : self.
               edit_record(new_fname.get(),fname, new_lname.get(), lname, new_uname.get(), uname, new_email.get(), email, new_subject.get(), subject, new_age.get(), age)).grid(row=14,column=2,sticky=W)
    ...
    def edit_record(self,new_fname,fname, new_lname, lname, new_uname, uname, new_email, email, new_subject, subject, new_age, age):
        query = 'UPDATE Student SET Firstname=?, Lastname=?, Username=?, Email=?, Subject=?, Age=? ' \
                'WHERE Firstname=? AND Lastname=? AND Username=? AND Email=? AND Subject=? AND Age=?'
        parameter = (new_fname, new_lname, new_uname, new_email, new_subject, new_age, fname, lname, uname, email, subject, age)
    ...
    def help(self):
        tkinter.messagebox.showinfo('Log', 'Reports sent')

    def exit(self):
        ex = tkinter.messagebox.askquestion('Exit', 'Want to close your application?')
        if ex == 'yes':
            root.destroy()
